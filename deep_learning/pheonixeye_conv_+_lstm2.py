# -*- coding: utf-8 -*-
"""PheonixEye_Conv_+_LSTM2.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1IdOAKhi42qKQQepCfw-SXws31jSDIFFD
...
# Define last_original_frame as the last frame in fire_sequence_large
...
# Select the sequence with the most fires from the validation dataset
example = val_dataset[max_idx]
...
# Use the sequence_with_most_fires for both the original and "predicted" frames
all_frames = sequence_with_most_fires

# First row for the original frames
axes_original = [plt.subplot(gs[i]) for i in range(5)]
for i, ax in enumerate(axes_original):
    ax.imshow(np.squeeze(all_frames[i]), cmap=custom_cmap, vmin=0, vmax=1)
    ax.set_title(f'Original Time {i+1}')

# Second row for the "predicted" frames (in this case, we only have original frames)
axes_predicted = [plt.subplot(gs[i+5]) for i in range(5)]
for i, ax in enumerate(axes_predicted):
    cmap_choice = custom_cmap if i != 4 else 'viridis'
    frame_choice = np.squeeze(all_frames[i])
    title_choice = 'Original Time' if i != 4 else 'Actual Prediction'
    ax.imshow(frame_choice, cmap=cmap_choice, vmin=0, vmax=1)
    ax.set_title(f'{title_choice} {i+1}')
...
# We use the last original frame and the last predicted frame for metric calculations
# In this case, we don't have a predicted frame, so let's use the last original frame as the "predicted" frame for demonstration
last_original_frame = all_frames[-1]
last_predicted_frame = all_frames[-1]
...
# Let's format and print the metrics as requested
